...
if ( comments_open() ) : 
	...
	comment_form( array(
		...
		'comment_field'				=>	'<label for="comment">' . __('Lời nhắn', 'qns') . '</label><textarea name="comment" id="comment" class="text_input" tabindex="4" rows="9" cols="60"></textarea>',
		...
		'title_reply'				=>	__( 'Xin để lại lời nhắn', 'qns' ),
		
		'title_reply_to'			=>	__( 'Xin để lại lời nhắn', 'qns' ),
		
		'cancel_reply_link'			=>	__( 'Cancel Reply To Comment', 'qns' ),
		
		'label_submit'				=>	__( 'Gửi tin', 'qns' ),
		
		'id_submit'					=>	'submit',
		
		'fields'					=>	array(
										
											'author'	=>	( $req ? '<label>' . __('Họ tên', 'qns') .' <span class="required">('.__('required', 'qns').')</span></label>' : '' ) . '<input id="author" class="text_input" name="author" type="text" value="' . esc_attr( $commenter['comment_author'] ) . '" placeholder="' . __( 'Name', 'qns' ) . '" ' . $aria_req . ' />',
